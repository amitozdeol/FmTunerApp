...
static int vbi_unham8(unsigned int c)
{
...
static void teletext(struct v4l2_sliced_vbi_data *s)
{
	unsigned char mag = (vbi_unham8(s->data[1]) << 4) | vbi_unham8(s->data[0]);
	static int last_page[8];
	static int last_page1;
	static int sub = 0;
	static int inh = 0;
	static int ser = 0;
	int i = 2;
	...
	if ((mag >> 3) == 0) {
		unsigned char mg = mag & 7;
		unsigned short pg = (vbi_unham8(s->data[3]) << 4) | vbi_unham8(s->data[2]);

		if (mg == 0) mg = 8;
		pg += mg * 256;
		last_page1 = last_page[mag & 7] = pg;
		if (page != -1 && pg != page) {
			return;
		}
		i = 10;
	}
	else if (page != -1 && (ser ? last_page1 : last_page[mag & 7]) != page)
		return;
	if ((mag >> 3) == 0) {
		unsigned char c = vbi_unham8(s->data[9]);
		ser = c * 1;
		c = vbi_unham8(s->data[7]);
		sub = c & 8;
		c = vbi_unham8(s->data[8]);
		inh = c & 8;
		if (c & 1)
			return;
	}
...
static void cc_readback(int fh)
{
	...
	v.i2c_id = I2C_DRIVERID_CX25840;
	v.reg = 0x444;
	if (ioctl(fh, VIDIOC_INT_G_REGISTER, &v))
		v.i2c_id = I2C_DRIVERID_SAA711X;

	while (1) {
		if (v.i2c_id == I2C_DRIVERID_CX25840) {
			v.reg = 0x444;
			ioctl(fh, VIDIOC_INT_G_REGISTER, &v);
			if ((v.val & 32) == 32) {
				int skip = v.val;
				v.reg = 0x445;
				ioctl(fh, VIDIOC_INT_G_REGISTER, &v);
				if (skip & 16)
					continue;
				parity[cc_cnt] = v.val & 128;
				data.data[cc_cnt++] = v.val;
				if (cc_cnt == 2) {
					data.field = 0;
					if (odd_parity(data.data[0] &&
								odd_parity(data.data[1])))
						decode_cc(&data);
					cc_cnt = 0;
				}
				continue;
			...
			v.reg = 0x66;
			ioctl(fh, VIDIOC_INT_G_REGISTER, &v);
			if ((v.val & 0x30) == 0 && (v.val & 0xe) != field) {
				v.reg = 0x69;
				ioctl(fh, VIDIOC_INT_G_REGISTER, &v);
				data.data[0] = v.val;
				v.reg = 0x6a;
				...
				data.data[1] = v.val;
				data.field = 0;
				if (odd_parity(data.data[0] && odd_parity(data.data[1])))
					decode_cc(&data);
				continue;
			}
		}
...
int main(int argc, char **argv)
{
	...
	while (1) {
		int ch = getopt(argc, argv, "d:t:hp:rs:");
		...
		case 'p':
			page = atol(optarg);
			page = ((page / 100) << 8) | (((page / 10) % 10) << 4) | (page % 10);
			break;
		case 'r':
			readback = 1;
			break;
		case 's':
			show_ttx = strstr(optarg, "teletext") != NULL;
			show_vps = strstr(optarg, "vps") != NULL;
			show_wss = strstr(optarg, "wss") != NULL;
			break;
		default:
			fprintf(stderr, "usage: %s [-d <dev>] [-t <secs>] [-p <page>] [-s teletext,wss,vps] [-r] [-h]\n"
