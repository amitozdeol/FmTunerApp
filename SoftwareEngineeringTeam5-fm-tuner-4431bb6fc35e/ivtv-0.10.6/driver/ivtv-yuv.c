				 ...
				 struct ivtvyuv_ioctl_dma_host_to_ivtv_args *args)
{
	...
	y_decode_height = uv_decode_height = args->src_h + args->src_y;
...
int ivtv_yuv_prep_frame(struct ivtv *itv, struct ivtvyuv_ioctl_dma_host_to_ivtv_args *args)
{
	...
	itv->yuv_info.new_frame_info[frame].src_x = args->src_x;
	itv->yuv_info.new_frame_info[frame].src_y = args->src_y;
	itv->yuv_info.new_frame_info[frame].src_w = args->src_w;
	itv->yuv_info.new_frame_info[frame].src_h = args->src_h;
	itv->yuv_info.new_frame_info[frame].dst_x = args->dst_x;
	itv->yuv_info.new_frame_info[frame].dst_y = args->dst_y;
	itv->yuv_info.new_frame_info[frame].dst_w = args->dst_w;
	itv->yuv_info.new_frame_info[frame].dst_h = args->dst_h;
	itv->yuv_info.new_frame_info[frame].tru_x = args->dst_x;
	itv->yuv_info.new_frame_info[frame].tru_w = args->srcBuf_width;
	itv->yuv_info.new_frame_info[frame].tru_h = args->srcBuf_height;
