...
static void ivtv_update_pgm_info(struct ivtv *itv)
{
	...
	while (i < cnt) {
		...
		struct ivtv_index_entry *e = itv->pgm_info + idx;
...
int ivtv_start_capture(struct file *filp)
{
...
unsigned int ivtv_v4l2_enc_poll(struct file *filp, poll_table * wait)
{
	...
	if (!eof && !test_bit(IVTV_F_S_STREAMING, &s->s_flags)) {
		int rc = ivtv_start_capture(filp);
...
int ivtv_v4l2_close(struct inode *inode, struct file *filp)
{
	...
	/* Stop decoding */
	if (test_bit(IVTV_F_S_STREAMING, &s->s_flags) && s->type >= IVTV_DEC_STREAM_TYPE_MPG) {
		IVTV_DEBUG_INFO("close stopping decode\n");

		ivtv_stop_v4l2_decode_stream(s, IVTV_STOP_FL_HIDE_FRAME, 0);
	}
	/* Stop capturing */
	else if (test_bit(IVTV_F_S_STREAMING, &s->s_flags)) {
		struct ivtv_stream *s_vbi = &itv->streams[IVTV_ENC_STREAM_TYPE_VBI];
