...
int ivtv_start_v4l2_encode_stream(struct ivtv_stream *s)
{
	...
	/* mute/unmute video */
	ivtv_vapi(itv, CX2341X_ENC_MUTE_VIDEO, 1, test_bit(IVTV_F_I_RADIO_USER, &itv->i_flags) ? 0x00808001 : 0);
...
static int ivtv_setup_v4l2_decode_stream(struct ivtv_stream *s)
{
	...
	ivtv_vapi(itv, CX2341X_DEC_SET_AUDIO_MODE, 2, 1, 0);
...
int ivtv_start_v4l2_decode_stream(struct ivtv_stream *s, int gop_offset)
{
	...
	if (itv->dec_options.speed.direction || itv->dec_options.speed.scale)
		ivtv_s_speed(itv, &itv->dec_options.speed);
...
int ivtv_stop_capture(struct ivtv_stream *s)
{
        struct ivtv *itv = s->itv;
	...
	/* Stop Capture Mode */
	if (s->type == IVTV_ENC_STREAM_TYPE_MPG && itv->end_gop == 1) {
		stopmode = 0;
	} else {
		stopmode = 1;
	}
...
int ivtv_stop_v4l2_decode_stream(struct ivtv_stream *s, int flags, u64 pts)
{
	...
	if (itv->dec_options.fast_stop == 1)
		flags &= ~IVTV_STOP_FL_WAIT_FOR_END;
	else if (itv->dec_options.fast_stop == 0)
		flags |= IVTV_STOP_FL_WAIT_FOR_END;

	IVTV_DEBUG_INFO("Stop Decode at %llu, flags: %x\n", pts, flags);

	/* Stop Decoder */
	if ((flags & IVTV_STOP_FL_WAIT_FOR_END) || pts) {
		u32 tmp = 0;
	...
	ivtv_vapi(itv, CX2341X_DEC_STOP_PLAYBACK, 3, flags & IVTV_STOP_FL_HIDE_FRAME, 0, 0);
	...
	itv->dec_options.speed.direction = 0;
	itv->dec_options.speed.scale = 0;
